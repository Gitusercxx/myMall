define(['tools'], function (tools) {
   ...
   let log_tool = {
      ...
      userlog: function () {
         ...
         if (checkval(tel, 'tel') && checkval($('.log_password').val(), 'password')) {
      ...
      userreg: function () {
         ...
         var pass_word =  $.md5($('.reg_pass_word').val());
         var pass_question = $('.pass_question option:selected').val();
         var pass_answer = $.md5($('.pass_answer').val());
         if ((password == pass_word||alert('密码不一致'))&&checkval(tel, 'tel') && checkval($('.reg_password').val(), 'password') && checkval($('.pass_answer').val(), 'pass_answer')) {
      ...
      show_question: function (d) {
         ...
         if (userinfor == 'unregistered') {
            ...
            $('.change_but').on('click', function () {
               var changepass = $.md5($('.changepass').val());
               var change_pass = $.md5($('.change_pass').val());
               var answer = $.md5($('.answer').val());
